...
### **Comparing tempograms** of a Sinfonia fast movement: clear overall tempo with regular vague areas due to hard to detect string note onsets.
...
The tempograms show the structure of the movement. Interesting in the structure is that both tempograms show regular parts in the A and B sections, in which it is difficult to estimate the tempo: the first one is marked as 'strings'. In the tempo curve you see note onsets are hard to detect here, because these parts are played by strings solo (which do not have a high amplitude attack phase) and in a lower tempo with lower dynamics. Each of these sections has a long rest before, after and in the middle of it. It lasts 3-5 counts (in the 3/8 meter). So the accented downbeat is 'missing' here: this might be regarded as a so-called 'loud rest'. These rests seem to cause the peaks on the tempo curve.
