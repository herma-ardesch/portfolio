...
### **Training Spotify** to classify features with even more precision!

![classification and Prediction](/2021 lopende zaken/studie/data/portfolio/classification and prediction.jpg)

***

In the previous tabs we showed visualisations, based on the Spotify API. Now let's find out if Spotify can classify this corpus according to the categories authentic or modern performance. For this purpose, we combined both authentic (A) and modern (M) playlists and asked Spotify to classify each performance. Not an easy task, because the playlists contain a variety of works, ranging from chamber music to symphonies!

First we tried to find out the most important features Spotify uses for this purpose and found that all twelve timbre features were on the top of the list to classify the performance. So using the timbre features, a prediction was calculated of how Spotify would perform - you find this prediction in the confusion matrix. Then we divided the works into five parts, thus training Spotify to perform better in each test-run. 

The calculated **precision** of the results was as follows: The first run resulted in a score of **0.86 (A)** vs. **0.85 (M)**, followed by a score of **0.86 (A)** vs. **0.86 (M)**, then a score of **0.88 (A)** vs. **0.83(!) (M)** and a last score of **0.89 (A)** vs. **0.87 (M)**.

Although the number of tests is limited, the precision is promising and it looks like Spotify is learning to identify performances with more precision! The plot shows two timbre components that played an important role in this task.
