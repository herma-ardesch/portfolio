...
Links to the [playlist used for authentic performances](https://open.spotify.com/playlist/2PmIxISPPF4ymvoa3bvCrh?si=UQbGeBYpRmSKUpqAj1W_ow) and the [playlist used for modern performances.](https://open.spotify.com/playlist/0F83MSl8bV2AxL2JNDTGew?si=n_uQMkPtSTG9tQ3JuYgJ0w)
...
The tempogram of the authentic performance of a Presto from the Sinfonia in D-major played by The English concert is compared to that of the modern performance by Musikcollegium Winterthur. Overall the authentic performance takes a slightly slower tempo of 120 BPM, whereas the modern performance is played at 126 BPM, as shown by the yellow lines on the tempograms.

The tempograms show the structure of the movement. Interesting in the structure is that both tempograms have a vague, vertical column in the A, A1, B and B1 sections, in which it is difficult to estimate the tempo; the first one is marked as 'strings'. In the tempo curve you see that note onsets are hard to detect here, because only strings (without a high amplitude attack phase) are playing here in lower tempo and dynamics. Each of these columns has a long rest before, after and in the middle of it. This rest lasts 3-5 beats (in the 3/8 meter). So the accented downbeat is 'missing': this might be regarded as a so-called 'loud rest'. These rests seem to cause the peaks on the tempo curve.

Apart from these slower, less loud and more reflective parts in this fast movement, Spotify does a good job in estimating the tempo.
...
### **Dynamic Time-Warping** to compare a Concerto in the versions for Flute to the version for Cello, played in a **much slower tempo.** 
...
### **Training Spotify** with the **RandomForest Model** to classify features with even more precision!

![Classification and Prediction](/2021 lopende zaken/studie/data/portfolio/classification and prediction.jpg)

***

The previous tabs showed visualisations, based on the Spotify API. Now let's find out if Spotify can classify the works in this corpus according to the categories authentic (A) or modern (M) performance. Not an easy task, because the playlists contain a variety of works, ranging from chamber music to symphonies!

First we tried to find out the most important features Spotify uses for this classification task and found that all twelve timbre features were on the top of the list. So using these timbre features, a prediction was calculated - you find this prediction in the confusion matrix. Then the complete corpus was divided into five parts, using the RandomForest Model to train Spotify to perform better in each run. 

The calculated **precision** of the results was as follows: The first run resulted in a (correct) score of **86% (A)** vs. **85% (M)**, followed by a score of **86% (A)** vs. **86% (M)**, then a score of **88% (A)** vs. **83%(!) (M)** and a last score of **89% (A)** vs. **87% (M)**.

Although the number of cross-validation runs is limited, the precision is promising and it looks like Spotify is learning to classify performances with more precision! The plot shows two timbre components that played an important role in this task.
...
In the visualisations on **playlist level** you can see that tempi in both groups were different, indeed, the most conspicuous difference being a greater variation of tempo in the authentic performances. I also found a greater variation in mood (valence - energy) in the authentic performances - a very important aspect, because C.P.E. Bach instructed performers of his music *'to be moved themselves, in order to transfer the emotions expressed in the music to the audience'* (quote from *'Versuch über die Wahre Art das Clavier zu spielen'*, C.P.E. Bach). 

On **track level** I found Spotify nicely identifying the lower pitch authentic instruments use in chromagrams. Two outliers in mood were analyzed in two Trimbre Self Similarity Matrices. They not only showed that the harpsichord's relative loudness results in quite a different timbre than that of the piano, but also that the different mechanics require a different interpretation technique. Tempograms and a short tempo curve of two individual tracks were compared and showed a similar structure, but slight differences in interpretation. The only features that Spotify did not identify correctly were key and chord analyses. However, for this purpose we only used the 24 minor and major chord and key templates to match the audio. Since C.P.E. Bach uses many, often unexpected harmonic turns that are far more complicated than the templates used, this is not surprising. This might however be a research topic to explore further in a machine-learning study!

A bit beside the research question, but helpful for musicians who wish to switch between two different performances and the score, I added a Dynamic Time Warping plot, in which two performances are aligned. Since Dynamic Time Warping is sensitive to differences in tuning, I used two different versions of the authentic playlist for this purpose as a bonus. Finally, I was nicely surprised by the promising precision of the Spotify API in the classification of performances and the possibility to improve this further by training! 

The **external validity** of this research has two aspects. C.P.E. Bach's 'Empfindsame Stil' represents the short transition of two style periods: from baroque to classicism. This limits his music to a relatively short period in time and thus the external validity of his music genre is also rather limited. But the research question focused on **comparing authentic performances to modern performances**. This comparison has a much broader scope than C.P.E. Bach's music. Especially because the Spotify API does a relatively good job to classify the two performance styles, the external validity of this research aspect is of greater importance and so this study ánd the Spotify API used might be useful for anyone comparing authentic to modern performances.

Concluding, I learned that the Spotify API is a great help to retrieve audio information on several levels, also for classical music. Visualizing these data in R helped me to understand better what is happening in a musical performance!
