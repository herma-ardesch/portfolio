...
The tempograms also show the structure of the movement. Interesting in the structure is that both tempograms have a vague, vertical column in the A, A1, B and B1 sections, in which it is difficult to estimate the tempo; the first one is marked as 'strings'. In the tempo curve you see that note onsets are hard to detect here, because only strings (without a high amplitude attack phase) play in a lower tempo and lower dynamics. Each of these columns has a long rest before, after and in the middle of it. The rests last 3-5 beats (in the 3/8 meter). They seem to cause spurious peaks on the tempo curve, similar to the high peaks at the beginning of each performance.
...
The previous tabs showed visualisations, based on the Spotify API. Now let's find out if Spotify can classify the works of this corpus in the right category of authentic (A) or modern (M) performance. Not an easy task, because the playlists contain a variety of works, ranging from chamber music to symphonies!

First we tried to find out the most important features Spotify uses for this classification task and found that all twelve timbre features were on top of the list. So using these timbre features, a prediction was calculated - you find this prediction in the confusion matrix. Then the complete corpus was divided into five parts, using the RandomForest Model to train Spotify to perform better in each run. 
...
The only features that Spotify did not identify correctly were key and chord analyses. However, for this purpose I only used the 24 minor and major chord and key templates to match the audio. Since C.P.E. Bach uses many, often unexpected harmonic turns that are far more complicated than the templates used, this is not surprising. This might however be a research topic to explore further in a machine-learning study to familiarize the computer with the capricious harmonic idiom of this genre.

A bit beside the research question, but helpful for musicians who wish to switch between two different performances and the score, I added a Dynamic Time Warping plot which aligned two performances as a bonus. Since Dynamic Time Warping is sensitive to differences in tuning, I used two versions of the same piece with quite different durations from the authentic playlist. Finally, I was nicely surprised by the promising precision of the Spotify API in the classification of performances and the possibility to improve this further by training! 
...
Concluding, I learned that the Spotify API can be a great help to retrieve audio information on several levels, also for classical music! Visualizing these data in R helped me to understand better what is happening in a musical performance.
