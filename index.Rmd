...
First we tried to find out the most important features Spotify uses for this classification task and found that all twelve timbre features were on top of the list. So using these timbre features, a prediction was calculated - you find this prediction in the confusion matrix. Then the complete corpus was divided into five parts, using the RandomForest Model for cross-validation and to train Spotify to perform better in each run. The calculated **precision** of the results was as follows:
...
1. C.P.E. Bach's music represents the **short transition of two style periods: from baroque to classicism.** This makes it a very specific style and limits the music to a relatively short period in time. This has a direct influence on the external validity of the findings in this portfolio: It is not really possible to generalize them, simply because C.P.E. Bach is the most important composer of the 'Empfindsame Stil'. 
